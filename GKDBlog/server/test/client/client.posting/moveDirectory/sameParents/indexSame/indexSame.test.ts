...
import {AddDirectoryDataType, AddFileDataType, MoveDirectoryDataType} from '@common/types'
...
/**
 * moveDirectory 에서 같은 부모에서 인덱스를 변경하지 않는 경우들을 테스트
 * - 여러 서브테스트를 통과해야 한다,.
 * - testOK 로 실행
 *
 * 초기 설정
 ...
 * 3. 마지막 -> 마지막
 */
export class IndexSame extends GKDTestBase {
  ...
  private async _1_first_to_first(db: Db, logLevel: number) {
    /**
     * 첫번째 자식폴더를 첫번째로 이동한다.
     *
     * 점검사항
    ...
    try {
      ...
      if (_parentSequence.join(',') !== '0,1,2,3,4') throw `1-2. 부모폴더: 자식폴더 순서가 0,1,2,3,4 가 아니라 ${_parentSequence.join(',')} 가 됬음`
  ...
  private async _2_middle_to_middle(db: Db, logLevel: number) {
    /**
     * 3번째 자식폴더를 3번째로 이동한다.
     * - 자식 폴더 순서가 01234 에서 01234 로 바뀐다.
     */
    try {
      const {jwtPayload, parentDirOId, parentDir} = this
  ...
  private async _3_last_to_last(db: Db, logLevel: number) {
    /**
     * 마지막 자식폴더를 마지막으로 이동한다.
     * - 자식 폴더 순서가 01234 에서 01234 로 바뀐다.
     */
    try {
      const {jwtPayload, parentDirOId, parentDir} = this
      const moveDirOId = parentDir.subDirOIdsArr[4]
...
if (require.main === module) {
  ...
  const testModule = new IndexSame(DEFAULT_REQUIRED_LOG_LEVEL) // __Test 대신에 모듈 이름 넣는다.
