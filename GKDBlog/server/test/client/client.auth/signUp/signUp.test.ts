...
export class SignUp extends GKDTestBase {
  private WorkCompletedTest: WorkCompleted
  private WrongInfoTest: WrongInfo
  ...
  constructor(REQUIRED_LOG_LEVEL: number) {
    ...
    this.WrongInfoTest = new WrongInfo(REQUIRED_LOG_LEVEL + 1)
  ...
  protected async execTest(db: Db, logLevel: number) {
    try {
      ...
      await this.WrongInfoTest.testOK(db, logLevel)
...
if (require.main === module) {
  ...
  const testModule = new SignUp(DEFAULT_REQUIRED_LOG_LEVEL) // __Test 대신에 모듈 이름 넣는다.
