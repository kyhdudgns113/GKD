...
export class GKDJwtService {
  ...
  async requestValidation(headers: any) {
    try {
      ...
      const uOId = jwtPayload.uOId
      const newHeader = generateRandomString(jwtHeaderLenVali)
      const newJwt = await this.jwtService.signAsync(jwtPayload)

      if (!this.uOIdToHeaderToUrl[uOId]) {
        this.uOIdToHeaderToUrl[uOId] = {}
      }
      this.uOIdToHeaderToUrl[uOId][newHeader] = url

      setTimeout(() => {
        delete this.uOIdToHeaderToUrl[uOId][newHeader]
      }, 3000)
  ...
  resetUOIdToHeaderToUrl(uOId: string, header: string) {
    delete this.uOIdToHeaderToUrl[uOId][header]
  }
  /**
   * 로그인에서 쓰지마 제발
   */
