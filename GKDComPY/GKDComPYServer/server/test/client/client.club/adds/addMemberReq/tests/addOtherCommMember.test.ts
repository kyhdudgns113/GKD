...
/**
 * 다른 공동체에 멤버를 추가하려고 하는것을 테스트한다.
 *   - 실패해야 한다.
 */
export class AddOtherCommMember extends GKDTestBase {
  ...
  protected async beforeTest(db: Db, logLevel: number) {
    try {
      ...
      const {clubOIdsArr, name: commName} = this.testDB.readComm(1).comm
...
if (require.main === module) {
  ...
  const testModule = new AddOtherCommMember(DEFAULT_REQUIRED_LOG_LEVEL)
