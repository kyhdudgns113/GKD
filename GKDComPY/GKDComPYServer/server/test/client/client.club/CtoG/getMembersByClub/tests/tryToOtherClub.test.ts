...
export class TryToOtherClub extends GKDTestBase {
  ...
  protected async execTest(db: Db, logLevel: number) {
    try {
      const {commOId, clubOId, jwtPayload, name} = this

      await this.db.collection('members').insertOne({commOId, clubOId, name})
      await this.portService.getMembersByClub(jwtPayload, clubOId)
      // BLANK LINE COMMENT:
    } catch (errObj) {
      // BLANK LINE COMMENT:
      throw errObj
