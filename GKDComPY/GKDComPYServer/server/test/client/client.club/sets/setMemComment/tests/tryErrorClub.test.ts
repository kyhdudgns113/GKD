...
/**
 * 클럽 0 에 있는 멤버인데, 클럽 1 의 OID 를 넘겨주는걸 테스트한다.
 *   - 실패해야 한다.
 */
export class TryErrorClub extends GKDTestBase {
  ...
  protected async beforeTest(db: Db, logLevel: number) {
    try {
      ...
      const {clubOId: clubOId1} = this.testDB.readClub(0, 1).club
  ...
  protected async finishTest(db: Db, logLevel: number) {
    try {
      const {commOId, name} = this
      await this.db.collection('members').deleteOne({commOId, name})
      // BLANK LINE COMMENT:
    } catch (errObj) {
      // BLANK LINE COMMENT:
      throw errObj
...
if (require.main === module) {
  ...
  const testModule = new TryErrorClub(DEFAULT_REQUIRED_LOG_LEVEL) // __Test 대신에 모듈 이름 넣는다.
