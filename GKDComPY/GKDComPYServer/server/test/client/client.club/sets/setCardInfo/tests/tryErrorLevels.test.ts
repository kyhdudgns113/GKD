...
export class TryErrorLevels extends GKDTestBase {
  ...
  protected async execTest(db: Db, logLevel: number) {
    ...
    try {
      ...
      await this.memberFail(this._level0Over.bind(this), db, logLevel)
      await this.memberFail(this._level0Under.bind(this), db, logLevel)
      await this.memberFail(this._level1Over.bind(this), db, logLevel)
      await this.memberFail(this._level1Under.bind(this), db, logLevel)
      await this.memberFail(this._level2Over.bind(this), db, logLevel)
      await this.memberFail(this._level2Under.bind(this), db, logLevel)
      // BLANK LINE COMMENT:
    } catch (errObj) {
      throw errObj
    }
  ...
  private async _level0Over(db: Db, logLevel: number) {
    try {
      const {jwtPayload, memOId} = this
      const posIdx = 0
      const name = '정민철'
      const skillIdxs = [0, 1, 2]
      const skillLevels = [6, 0, 0]
      const data: SetCardInfoDataType = {memOId, posIdx, name, skillIdxs, skillLevels}
  ...
  private async _level0Under(db: Db, logLevel: number) {
    try {
      const {jwtPayload, memOId} = this
      const posIdx = 0
      const name = '정민철'
      const skillIdxs = [0, 1, 2]
      const skillLevels = [-1, 0, 0]
      const data: SetCardInfoDataType = {memOId, posIdx, name, skillIdxs, skillLevels}
  ...
  private async _level1Over(db: Db, logLevel: number) {
    try {
      const {jwtPayload, memOId} = this
      const posIdx = 0
      const name = '정민철'
      const skillIdxs = [0, 1, 2]
      const skillLevels = [0, 6, 0]
      const data: SetCardInfoDataType = {memOId, posIdx, name, skillIdxs, skillLevels}
  ...
  private async _level1Under(db: Db, logLevel: number) {
    try {
      const {jwtPayload, memOId} = this
      const posIdx = 0
      const name = '정민철'
      const skillIdxs = [0, 1, 2]
      const skillLevels = [0, -1, 0]
      const data: SetCardInfoDataType = {memOId, posIdx, name, skillIdxs, skillLevels}
  ...
  private async _level2Over(db: Db, logLevel: number) {
    try {
      const {jwtPayload, memOId} = this
      const posIdx = 0
      const name = '정민철'
      const skillIdxs = [0, 1, 2]
      const skillLevels = [0, 0, 6]
      const data: SetCardInfoDataType = {memOId, posIdx, name, skillIdxs, skillLevels}
  ...
  private async _level2Under(db: Db, logLevel: number) {
    try {
      const {jwtPayload, memOId} = this
      const posIdx = 0
      const name = '정민철'
      const skillIdxs = [0, 1, 2]
      const skillLevels = [0, 0, -1]
      const data: SetCardInfoDataType = {memOId, posIdx, name, skillIdxs, skillLevels}
...
if (require.main === module) {
  ...
  const testModule = new TryErrorLevels(DEFAULT_REQUIRED_LOG_LEVEL)
