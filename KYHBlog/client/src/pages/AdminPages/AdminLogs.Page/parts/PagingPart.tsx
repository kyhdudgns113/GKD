...
import {useAdminStatesContext} from '@context'
...
export const PagingPart: FC<PagingPartProps> = ({pageIdx, pageTenIdx, setPageIdx, setPageTenIdx, className, style, ...props}) => {
  const {logArr} = useAdminStatesContext()
    ...
    <div className={`Paging_Part ${className || ''}`} style={style} {...props}>
      ...
      {Array(10)
        ...
        .map((_, idx) => {
          ...
          // 초과 시 null 반환
          if (nowIdx * ADMIN_LOG_PER_PAGE > logArr.length) {
            return null
          }
      ...
      {/* 3. 다음 pageTenIdx */}
      {pageTenIdx < Math.floor(logArr.length / ADMIN_LOG_PER_PAGE) - 1 && (
